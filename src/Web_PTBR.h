...
<body>
 <div class="setup-title">
  <h1>DeskBuddy</h1>
  <p>Conecte ao seu WiFi para começar</p>
 </div>

 <form action="/save-credentials" method="POST">
 <div class="card">
  <div class="step"><span class="step-num">1</span><h2>Rede WiFi</h2></div>
  <div class="field-group">
   <label class="field-label">Nome da Rede (SSID)</label>
   <div class="ssid-row">
    <input type="text" name="wifiSsid" id="wifiSsid" class="settings-input" placeholder="ex.: Marina_2.4G" autocomplete="off" autocapitalize="none" spellcheck="false">
    <button type="button" class="btn-scan" id="scanBtn" onclick="scanWifi()">Buscar</button>
   </div>
   <div id="apList"></div>
  </div>
  <div class="field-group">
   <label class="field-label">Senha</label>
   <input type="password" name="wifiPass" id="wifiPass" class="settings-input" placeholder="Senha do WiFi" autocomplete="off">
   <span class="field-help">Deixe em branco para manter a senha atual, se já configurada.</span>
  </div>
 </div>

 <div class="card">
  <div class="step"><span class="step-num">2</span><h2>Configuração de IP</h2></div>
  <div class="field-group">
   <div style="display:flex;justify-content:space-between;align-items:center">
    <label class="field-label" style="margin:0">Usar IP estático</label>
 ...
 <option value="0">Nao (DHCP)</option>
 <option value="1">Sim</option>
</select>
 </div>
 </div>
 <div id="staticFields">
   <div class="field-row">
    <div><label class="field-label">Endereço IP</label><input type="text" name="wifiIp" id="wifiIp" class="settings-input" placeholder="192.168.1.100" autocomplete="off" autocapitalize="none" spellcheck="false"></div>
   ...
   <div class="field-row">
    <div><label class="field-label">Máscara</label><input type="text" name="wifiSubnet" id="wifiSubnet" class="settings-input" placeholder="255.255.255.0" autocomplete="off" autocapitalize="none" spellcheck="false"></div>
    <div><label class="field-label">DNS primário</label><input type="text" name="wifiDns1" id="wifiDns1" class="settings-input" placeholder="1.1.1.1" autocomplete="off" autocapitalize="none" spellcheck="false"></div>
   </div>
 ...
 <div class="card" style="text-align:center">
  <button type="submit" class="btn">Salvar &amp; Conectar</button>
  <p class="help">Após conectar, abra o IP do dispositivo para configurar MQTT, chaves de API e mais.</p>
 </div>
 </form>
 ...
 function scanWifi() {
  ...
  fetch('/wifi-scan').then(function(r) { return r.json(); }).then(function(aps) {
   ...
   if (!aps.length) { list.innerHTML = '<div class="ap-item"><span class="ap-name" style="color:#64748b;">Nenhuma rede encontrada</span></div>'; return; }
   var html = '';
   for (var i = 0; i < aps.length; i++) {
    var pct = rssiToPercent(aps[i].rssi);
    var bars = signalBars(pct);
 ...
 function selectAP(el) {
  ...
  if (el.dataset.secure === '0') { pw.value = ''; pw.placeholder = 'Rede aberta - sem senha'; }
  else { pw.placeholder = 'Senha do WiFi'; }
 }
 ...
 document.querySelector('form').addEventListener('submit', function(e) {
  if (!confirm('Salvar e conectar o DeskBuddy?')) e.preventDefault();
 });
 </script>
</body>
</html>
